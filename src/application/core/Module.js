...
export function constructLocalized(selectors) {
    ...
    return (module) => {
        ...
        Object.keys(derivedSelectors).forEach(key => {
            ...
            localizedSelectors[key] = (globalState) => selector(globalState[module.store], globalState)
        })
...
export default class Module {
    ...
    reducer(state, action) {
        ...
        if (this.modulesReducer) {
            return {
                ...this.modulesReducer(
                    this.modulesState(this.options.reducer(state, action, this), this.modules),
                    action
                ),
            };
        }
    ...
    modulesState(state, modules) {
        return modules.reduce(function(actualState, module) {
                if (module.store in state) {
                    return {
                        ...actualState,
    ...
    extend(options) {
        ...
        return new Module({
            ...
            reducer: (state, action, module) => {
                state = parent.reducer(state, action, module)
                if (isUndefined(options.reducer)) {
                    ...
                    if (this.modulesReducer) {
                        return {
                            ...state,
                            ...this.modulesReducer(this.modulesState(state, this.modules), action),
                        };
                    }

                    return options.reducer(state, action, module)
                }
            },
            modules: modules,
            sagas: module => {
