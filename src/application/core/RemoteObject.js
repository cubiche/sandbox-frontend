...
export default function createModule({ namespace, store, initialState={ data: [] } }) {
    return new Module({
        ...
        selectors: (module) => ({
            ...
            errors: new Module.Selector(selectors => state => {
                ...
                const errors = error.graphQLErrors.filter(
                    item => item.code !== 401 && item.code !== 422
                );

                return errors.map(item => item.message);
            }),
            accesDenied: new Module.Selector(selectors => state => {
                const error = selectors.error(state);
                if (error === undefined) {
                    return [];
                }
                ...
                return error.graphQLErrors.filter(item => item.code === 401).map(item => item.message);
            }),
            validationErrors: new Module.Selector(selectors => state => {
                const error = selectors.error(state);
                if (error === undefined) {
                    return {};
                }
                ...
                return error.graphQLErrors
                    .filter(item => item.code === 422)
                    .reduce((errors, error) => {
                        return [...errors, ...error.errors];
                    }, [])
                    .reduce((formattedErrors, error) => {
                        return { ...formattedErrors, [error.path]: error.message };
                    }, {});
            })
        }),
